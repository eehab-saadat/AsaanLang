    ...
    def tokenizer(self, exprs):
        ...
        for i in range(len(exprs)):
            ...
            if type(exprs[i])==list:
                ...
                if exprs[i] in OPERATOR.arithematic_op or exprs[i] in OPERATOR.assignment_op:

                    exprs[i] = OPERATOR(exprs[i])
                # check for relational operators
                elif exprs[i] in OPERATOR.relational_op:

                    exprs[i]=OPERATOR(exprs[i])

                # check boolean keywords
                elif exprs[i] in BOOL_KW.bool_val:

                    exprs[i] = BOOL_KW(exprs[i])
                # check for conditional keywords
                elif exprs[i] in self.conditional_keyword:

                    exprs[i] = KEYWORD(exprs[i],self.conditional_keyword[exprs[i]])
                    self.tokenizer(exprs[i+1])
                    exprs[i+1] = CONDITIONAL_EXP(exprs[i+1])
                # check for other keywords
                elif exprs[i] in self.other_keywords:

                    exprs[i] = KEYWORD(exprs[i], self.other_keywords[exprs[i]])
                # check for comments
                elif type(exprs[i])==str:
                    if("#" in exprs[i]):
                        exprs[i] = COMMENT(exprs[i])
                    # symbol table checking
                        ...
                        if exprs[i].lower() == "number":
                            self.symbol_table.append(NUMBER(exprs[i+1]))
                        elif exprs[i].lower() == "ishariya":
                            self.symbol_table.append(ISHARIYA(exprs[i+1]))
                        elif exprs[i].lower() == "lafz":
                            self.symbol_table.append(LAFZ(exprs[i+1]))
                        elif exprs[i].lower() == "boolean":
                            self.symbol_table.append(BOOLEAN(exprs[i+1]))
